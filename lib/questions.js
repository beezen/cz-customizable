...
module.exports = {
  getQuestions(config, cz) {
    ...
    messages.requirementId = messages.requirementId || 'Requirement list number:\n';
    messages.body =
    ...
    let questions = [
      ...
      },
      {
        ...
        validate(value) {
          ...
          return new Error('任务单号格式不正确');
        },
      },
      {
        type: 'input',
        name: 'requirementId',
        message: messages.requirementId,
        validate(value) {
          if (!isInputRequired.includes('requirementId')) return true;
          if (/^.+$/.test(value)) {
            return true;
          }
          return new Error('需求编号格式不正确');
        },
      },
      {
        type: 'input',
